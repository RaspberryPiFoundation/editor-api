  ...
  class ImportBatch
    class << self
      ...
      def validate_school_data(data, row_number)
        ...
        # Strip whitespace from owner_email
        data[:owner_email] = data[:owner_email]&.strip
      ...
      def enqueue_import_job(schools_data:, user_id:, token:)
        ImportSchoolsJob.perform_later(
