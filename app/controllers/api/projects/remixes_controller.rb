...
module Api
  module Projects
    class RemixesController < ApiController
      ...
      def project
        scope = params[:project_type] == Project::Types::SCRATCH ? Project.unscoped : Project
        @project ||= scope.find_by!(identifier: params[:project_id])
      end

      def load_and_authorize_remix
        scope = params[:project_type] == Project::Types::SCRATCH ? Project.unscoped : Project
        @project = scope.find_by!(remixed_from_id: project.id, user_id: current_user&.id)
