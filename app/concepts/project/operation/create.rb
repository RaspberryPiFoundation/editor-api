    ...
    class Create
      ...
      class << self
        def call(user_id:, params:)
          ...
          project = DEFAULT_PROJECT.merge(params)
        ...
        def create_with_content(project, user_id)
          new_project = Project.new(project_type: project[:type], user_id: user_id, name: 'Untitled Project')
          new_project.components.build(project[:components])
          new_project
        end
      end
