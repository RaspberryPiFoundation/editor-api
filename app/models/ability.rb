...
class Ability
  ...
  def define_school_teacher_abilities(user:, school:)
    ...
    can(%i[read create create_batch destroy], ClassStudent, school_class: { id: SchoolClass.with_class_teacher(user.id).select(:id), school: { id: school.id } })
    can(%i[read create create_batch destroy], ClassTeacher, school_class: { id: SchoolClass.with_class_teacher(user.id).select(:id), school: { id: school.id } })
