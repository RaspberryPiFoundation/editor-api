...
RSpec.describe School::ImportBatch do
  describe '.call' do
    ...
    before do
      allow(ImportSchoolsJob).to receive(:perform_later).and_return(
        instance_double(ImportSchoolsJob, job_id: 'test-job-id')
      )
    end
    ...
    context 'with valid CSV' do
      ...
      it 'enqueues ImportSchoolsJob' do
        ...
        expect(ImportSchoolsJob).to have_received(:perform_later).with(
          hash_including(
    ...
    context 'with invalid website format' do
      let(:csv_content) do
        <<~CSV
          name,website,address_line_1,municipality,country_code,owner_email
          Test School,not-a-url,123 Main St,Springfield,US,owner@example.com
        CSV
      end
    ...
    context 'with missing required fields' do
      let(:csv_content) do
        <<~CSV
          name,website,address_line_1,municipality,country_code,owner_email
          ,https://test.example.com,123 Main St,Springfield,US,owner@example.com
        CSV
      end
