...
RSpec.describe SchoolOwnershipMailer do
  ...
  describe 'complete_ownership_transfer' do
    subject(:email) { described_class.with(ownership_transfer:).complete_ownership_transfer }
    ...
    it 'is sent to the new owner' do
      expect(email.to).to eq([ownership_transfer.email_address])
    end

    it 'includes the new owner\'s name in the body' do
      expect(email.body.to_s).to include(nominee.name)
    end
    ...
    context 'when the new owner is missing from the user-info response' do
      before do
        stub_user_info_api_fetch_by_ids(
          user_ids: [nominee.id, requested_owner.id],
          users: [{ id: requested_owner.id, name: requested_owner.name }]
