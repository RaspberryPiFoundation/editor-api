...
RSpec.describe 'School project submit requests', type: :request do
  ...
  context 'when logged in as student' do
    ...
    context 'when attempting an invalid status transition' do
      ...
      it 'returns error message' do
        expect(JSON.parse(response.body)['error']).to eq("Cannot transition from 'complete' to 'submitted'")
      end
  ...
  context 'when project does not exist' do
    before do
      authenticated_in_hydra_as(student)
      post('/api/projects/does-not-exist/submit', headers:)
    end
