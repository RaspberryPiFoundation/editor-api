...
RSpec.describe 'School project unsubmit requests', type: :request do
  ...
  context 'when logged in as student' do
    before do
      ...
      post("/api/projects/#{student_project.identifier}/unsubmit", headers:)
    end
    ...
    it 'sets the status to unsubmitted' do
  ...
  context 'when logged in as teacher' do
    ...
    context 'when attempting an invalid status transition' do
      before do
        student_project.school_project.transition_status_to!(:submitted, student.id)
        student_project.school_project.transition_status_to!(:returned, teacher.id)
        post("/api/projects/#{student_project.identifier}/unsubmit", headers:)
      end
      ...
      it 'returns error message' do
        expect(JSON.parse(response.body)['error']).to eq("Cannot transition from 'returned' to 'unsubmitted'")
      end

      it 'does not change the status' do
        expect(student_project.school_project).to be_returned
      end
    end
  end
  ...
  context 'when project does not exist' do
    before do
      authenticated_in_hydra_as(student)
      post('/api/projects/does-not-exist/unsubmit', headers:)
    end
